...
export default function TarotPage() {
  ...
  const faqs = [
    { q: 'Как работает расклад таро с AI?', a: 'Вы задаёте вопрос, AI-консультант Сатья вытягивает карты из колоды Анализ двух людей Rider-Waite и даёт персональную интерпретацию именно для вашей ситуации.' },
    ...
    { q: 'Сколько карт в раскладе?', a: 'Сатья адаптирует расклад под вопрос: 1 карта для простого ответа, 3 карты для ситуации (прошлое/настоящее/будущее), 5 карт для глубокого анализа.' },
    { q: 'Насколько точны расклады?', a: 'Совместимость — это инструмент рефлексии и интуиции. Многие пользователи отмечают высокую точность попаданий. Результат зависит от конкретности вашего вопроса.' },
  ]
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        ::-webkit-scrollbar{width:3px}::-webkit-scrollbar-thumb{background:rgba(192,112,255,0.3);border-radius:2px}
      ...
      <div style={{position:'fixed',inset:0,zIndex:0,pointerEvents:'none',overflow:'hidden'}}>
        ...
        <div style={{position:'absolute',top:'-10%',left:'30%',width:'60vw',height:'60vw',maxWidth:'700px',borderRadius:'50%',background:'radial-gradient(circle,rgba(192,112,255,0.08) 0%,transparent 70%)'}}/>
      </div>
      ...
      <div style={{position:'relative',zIndex:10}}>
        ...
        <nav style={{padding:'20px 52px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid rgba(255,255,255,0.06)'}} className="pad">
          ...
          <div style={{display:'flex',gap:'28px',alignItems:'center'}}>
            {[['Совместимость','/compatibility'],['Астрология','/astrology'],['Нумерология','/numerology'],['Совместимость','/compatibility'],['Премиум','/premium']].map(([l,h])=>(
          ...
          <div className="hero-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'60px',alignItems:'center'}}>
            <div style={{animation:'fadeUp 0.7s ease both'}}>
              ...
              <h1 style={{fontFamily:'"Playfair Display",serif',fontSize:'clamp(36px,5vw,64px)',fontWeight:900,color:'#FFFFFF',lineHeight:1.1,marginBottom:'20px'}}>
                Совместимость<br/><span style={{color:'rgba(192,112,255,0.9)'}}>Расклад</span><br/>на любой вопрос
              </h1>
              <p style={{fontFamily:'"Lora",serif',fontSize:'17px',lineHeight:1.85,color:'rgba(210,200,245,0.65)',marginBottom:'36px'}}>
                Сатья — Сатья анализирует совместимость пары через нумерологию и астрологию. Узнайте о сильных сторонах союза, зонах роста и потенциале ваших отношений.
              </p>
              ...
              <div style={{display:'flex',gap:'28px'}}>
                {[['78','арканов'],['3-5','карт в раскладе'],['5','бесплатных сообщений']].map(([v,l])=>(
                  <div key={l}>
                    <div style={{fontFamily:'"Playfair Display",serif',fontSize:'22px',fontWeight:800,color:'rgba(192,112,255,0.9)'}}>{v}</div>
              ...
              <div style={{position:'relative',width:'280px',height:'420px'}}>
                {[{r:-12,y:-20,x:-30,z:0},{r:6,y:-10,x:20,z:1},{r:0,y:0,x:0,z:2}].map((c,i)=>(
                  <div key={i} style={{
                    ...
                    zIndex:c.z,
                    background:'linear-gradient(145deg,rgba(192,112,255,0.15),rgba(60,20,100,0.8))',
                    ...
                    {i===2 && <div style={{textAlign:'center'}}>
                      ...
                      <div style={{fontFamily:'"Playfair Display",serif',fontSize:'22px',fontWeight:900,color:'rgba(192,112,255,0.9)'}}>Совместимость</div>
                      <div style={{fontFamily:'"Lora",serif',fontSize:'13px',fontStyle:'italic',color:'rgba(192,112,255,0.5)',marginTop:'6px'}}>Анализ двух людей</div>
                    </div>}
                  </div>
                ))}
              </div>
          ...
          <div style={{maxWidth:'1000px',margin:'0 auto'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'24px'}}>
              {[
                ...
                {num:'02',icon:'💫',title:'Карты открываются',desc:'Сатья вытягивает карты специально для вашей ситуации. Каждая карта — это зеркало вашей реальности.'},
                ...
                <div key={i} style={{padding:'28px',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(192,112,255,0.12)',borderRadius:'14px',position:'relative',overflow:'hidden'}}>
                  <div style={{position:'absolute',top:0,left:0,right:0,height:'2px',background:'linear-gradient(90deg,transparent,rgba(192,112,255,0.5),transparent)'}}/>
                  <div style={{fontFamily:'"Playfair Display",serif',fontSize:'11px',letterSpacing:'3px',color:'rgba(192,112,255,0.4)',marginBottom:'14px'}}>{s.num}</div>
        ...
        <section style={{padding:'80px 52px'}} className="pad">
          <div style={{maxWidth:'1000px',margin:'0 auto'}}>
            <h2 style={{fontFamily:'"Playfair Display",serif',fontSize:'clamp(24px,3.5vw,40px)',fontWeight:900,color:'#FFFFFF',marginBottom:'40px',textAlign:'center'}}>Что умеет Сатья</h2>
          ...
          <div style={{maxWidth:'600px',margin:'0 auto',textAlign:'center'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginBottom:'24px'}}>
              <div style={{padding:'28px',background:'rgba(192,112,255,0.05)',border:'1px solid rgba(192,112,255,0.2)',borderRadius:'14px'}}>
                ...
                <Link href="/auth/register" style={{display:'block',padding:'11px',textAlign:'center',borderRadius:'8px',background:'rgba(192,112,255,0.15)',border:'1px solid rgba(192,112,255,0.3)',fontFamily:'"Playfair Display",serif',fontSize:'12px',fontWeight:700,color:'rgba(192,112,255,0.9)',textDecoration:'none',letterSpacing:'0.5px'}}>Выбрать</Link>
              </div>
        ...
        <section style={{padding:'80px 52px'}} className="pad">
          <div style={{maxWidth:'900px',margin:'0 auto'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'16px'}}>
              {reviews.map((r,i)=>(
                <div key={i} style={{padding:'24px',background:'rgba(255,255,255,0.025)',border:'1px solid rgba(192,112,255,0.1)',borderRadius:'14px'}}>
                  <div style={{color:'rgba(192,112,255,0.8)',fontSize:'16px',marginBottom:'12px'}}>★★★★★</div>
                  ...
                  <div style={{fontFamily:'"Playfair Display",serif',fontSize:'13px',fontWeight:700,color:'rgba(192,112,255,0.7)'}}>{r.name}</div>
