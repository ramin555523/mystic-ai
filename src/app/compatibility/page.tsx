...
export default function CompatibilityPage() {
  const reviews = [
    { text: 'Сатья описала наши отношения с мужем так точно — сильные стороны и сложности совпали один в один.', name: 'Карина М.', city: 'Київ' },
    { text: 'Анализ совместимости с деловым партнёром помог понять почему мы так хорошо работаем вместе.', name: 'Оксана Д.', city: 'Варшава' },
    { text: 'Спросила про отношения с мамой — Сатья объяснила наши конфликты через числа. Стало намного легче.', name: 'Аліна П.', city: 'Лондон' },
  ]

  const faqs = [
    { q: 'Можно анализировать любые отношения?', a: 'Да — романтические, дружеские, семейные, деловые. Сатья анализирует любой тип связи между двумя людьми.' },
    { q: 'Нужны ли данные второго человека?', a: 'Для полного анализа да. Минимум: имя и дата рождения. Чем больше данных, тем точнее результат.' },
    { q: 'Что если совместимость низкая?', a: 'Низкая совместимость — не приговор. Это области для роста. Сатья даёт практические советы.' },
    { q: 'Можно анализировать несколько пар?', a: 'Да — каждый запрос это отдельный анализ. Можно сравнить нескольких кандидатов.' },
  ]
                ...
                {[{r:-12,y:-20,x:-30,z:0},{r:6,y:-10,x:20,z:1},{r:0,y:0,x:0,z:2}].map((c,i)=>(
                  <div key={i} style={{
                    ...
                    {i===2 && <div style={{textAlign:'center'}}>
                      ...
                      <div style={{fontFamily:'"Playfair Display",serif',fontSize:'22px',fontWeight:900,color:'rgba(192,112,255,0.9)'}}>Таро</div>
